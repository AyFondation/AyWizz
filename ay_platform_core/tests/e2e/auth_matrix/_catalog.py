...
_C2_AUTH: list[EndpointSpec] = [
    ...
    EndpointSpec(
        component="c2_auth",
        method="POST",
        path="/admin/tenants/{tenant_id}/deactivate",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        component="c2_auth",
        method="POST",
        path="/admin/tenants/{tenant_id}/reactivate",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        component="c2_auth",
        method="POST",
        path="/admin/users/{user_id}/reactivate",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
...
_C7_MEMORY: list[EndpointSpec] = [
    ...
    EndpointSpec(
        component="c7_memory",
        method="POST",
        path="/api/v1/memory/projects/{project_id}/sources/{source_id}/ingest-chunks",
        auth=Auth.ROLE_GATED,
        scope=Scope.PROJECT,
        success_status=201,
        accept_roles=("project_editor", "project_owner"),
        ...
        notes=(
            "R-400-223 v2 — pure-INSERT path for C13-produced chunks "
            "(D-020). Accepts ChunkRich items + embedding model "
            "metadata; persists Arango rows without invoking C7's "
            "embedder. Backward-compat fallback embeds locally when "
            "ChunkRich.embedding is None (removed v2)."
        ),
    ),
    EndpointSpec(
        component="c7_memory",
    ...
    EndpointSpec(
        ...
        backend_collection="memory_kg_entities",
        notes=(
            "V2 #3-A.a (R-400-200) — DETERMINISTIC schema-guided L1 "
            "extraction (closed ontology E-400-006) on an existing "
            "requirements-spec source. No LLM. Persists L1 entities/edges "
            "to memory_kg_entities / memory_kg_relations with layer + "
            "ontology_version."
        ),
    ),
    EndpointSpec(
    ...
    # D-020 session 7 — `/reprocess` EndpointSpec removed alongside the
    # route + service method. Reprocess responsibility moved to C12
    # (the n8n workflow re-triggers C13 → C7 /ingest-chunks).
    EndpointSpec(
        component="c7_memory",
        method="POST",
        path="/api/v1/memory/projects/{project_id}/reembed",
        auth=Auth.ROLE_GATED,
        scope=Scope.PROJECT,
        success_status=200,
        accept_roles=("project_owner",),
...
_C8_ADMIN: list[EndpointSpec] = [
    ...
    EndpointSpec(
        component="c8_admin",
        method="PUT",
        path="/admin/v1/llm/registry/{model_id}",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        component="c8_admin",
        method="PUT",
        path="/admin/v1/llm/providers/{provider_id}",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        component="c8_admin",
        method="PUT",
        path="/admin/v1/llm/providers/{provider_id}/api-key",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        component="c8_admin",
        method="GET",
        path="/admin/v1/llm/embedding-providers",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        ...
        accept_global_roles=("platform_manager",),
        backend=Backend.ARANGO,
        backend_collection="embedding_providers",
    ),
    EndpointSpec(
        component="c8_admin",
        method="PUT",
        path="/admin/v1/llm/embedding-providers/{provider_id}",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        component="c8_admin",
        method="PUT",
        path="/admin/v1/llm/embedding-providers/{provider_id}/api-key",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        ...
        backend=Backend.ARANGO,
        backend_collection="embedding_providers",
        notes="409 while any embedding model still references the provider.",
    ),
    EndpointSpec(
        component="c8_admin",
        method="GET",
        path="/admin/v1/llm/embedding-models",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        ...
        accept_global_roles=("platform_manager",),
        backend=Backend.ARANGO,
        backend_collection="embedding_models",
    ),
    EndpointSpec(
        component="c8_admin",
        method="PUT",
        path="/admin/v1/llm/embedding-models/{model_id}",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        ...
        backend=Backend.ARANGO,
        backend_collection="embedding_models",
        notes="409 while a project still selects the model.",
    ),
    # Per-tenant EMBEDDING catalogue + project selection (D-011) — admin /
    # tenant_admin, tenant-scoped, platform_manager EXCLUDED (content op).
    EndpointSpec(
        component="c8_admin",
        method="GET",
        path="/api/v1/llm/embedding-catalog",
        auth=Auth.ROLE_GATED,
        scope=Scope.TENANT,
        success_status=200,
        accept_global_roles=("admin", "tenant_admin"),
    ...
    EndpointSpec(
        component="c8_admin",
        method="GET",
        path="/api/v1/llm/embedding-catalog/available",
        auth=Auth.ROLE_GATED,
        scope=Scope.TENANT,
        success_status=200,
        accept_global_roles=("admin", "tenant_admin"),
        excluded_global_roles=("platform_manager",),
        notes=(
            "Add picker — platform-registry embedding models not yet in the "
            "tenant catalogue (the registry list itself is platform_manager-only)."
        ),
    ),
    EndpointSpec(
        component="c8_admin",
        method="PUT",
        path="/api/v1/llm/embedding-catalog/{model_id}",
        auth=Auth.ROLE_GATED,
        scope=Scope.TENANT,
        success_status=200,
        accept_global_roles=("admin", "tenant_admin"),
        excluded_global_roles=("platform_manager",),
        backend=Backend.ARANGO,
        backend_collection="embedding_catalog",
    ),
    EndpointSpec(
        component="c8_admin",
        method="DELETE",
        ...
        backend=Backend.ARANGO,
        backend_collection="embedding_catalog",
        notes="409 while a project in the tenant still selects the model.",
    ),
    EndpointSpec(
        component="c8_admin",
        method="GET",
        path="/api/v1/llm/projects/{project_id}/embedding",
        auth=Auth.ROLE_GATED,
        scope=Scope.TENANT,
        success_status=200,
        accept_global_roles=("admin", "tenant_admin"),
    ...
    EndpointSpec(
        ...
        excluded_global_roles=("platform_manager",),
        backend=Backend.ARANGO,
        backend_collection="embedding_project_selection",
    ),
    # Global LLM quota policy (Lot 3) — platform_manager platform operator.
    EndpointSpec(
        component="c8_admin",
        method="GET",
        path="/admin/v1/quota/policy",
        auth=Auth.ROLE_GATED,
        scope=Scope.NONE,
        success_status=200,
        accept_global_roles=("platform_manager",),
    ...
    EndpointSpec(
        component="c8_admin",
        method="GET",
        path="/api/v1/llm/catalog/resolve",
        auth=Auth.AUTHENTICATED,
        scope=Scope.TENANT,
        success_status=200,
    ),
    EndpointSpec(
        component="c8_admin",
        method="PUT",
        path="/api/v1/llm/catalog/{model_id}",
        auth=Auth.ROLE_GATED,
        scope=Scope.TENANT,
        success_status=200,
        accept_global_roles=("admin", "tenant_admin"),
    ...
    # Per-project model list (lazy default from the tenant catalogue).
    EndpointSpec(
        component="c8_admin",
        method="GET",
        path="/api/v1/llm/projects/{project_id}/models",
        auth=Auth.ROLE_GATED,
        scope=Scope.TENANT,
        success_status=200,
        accept_global_roles=("admin", "tenant_admin"),
