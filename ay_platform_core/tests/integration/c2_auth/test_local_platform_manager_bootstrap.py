# =============================================================================
# File: test_local_platform_manager_bootstrap.py
# Version: 1
# Path: ay_platform_core/tests/integration/c2_auth/test_local_platform_manager_bootstrap.py
# Description: Verifies the C2 application platform_manager bootstrap path —
...
#                   `local_platform_manager_*` fields are non-empty, the
#                   helper creates a user with role PLATFORM_MANAGER.
...
from ay_platform_core.c2_auth.main import _ensure_local_platform_manager
    ...
    return AuthConfig.model_validate(
        {
            ...
            "local_platform_manager_username": tm_username,
            "local_platform_manager_password": tm_password,
        }
    )


@pytest.mark.asyncio
async def test_platform_manager_user_is_created_with_correct_role(
    tm_repo: AuthRepository,
) -> None:
    cfg = _build_config()
    assert await tm_repo.get_user_by_username(_TM_USER) is None

    await _ensure_local_platform_manager(tm_repo, cfg)

    user = await tm_repo.get_user_by_username(_TM_USER)
    assert user is not None
    assert user.username == _TM_USER
    role_values = [r.value for r in user.roles]
    assert "platform_manager" in role_values, (
        f"expected platform_manager role, got {role_values}"
    )
...
async def test_no_op_when_username_empty(tm_repo: AuthRepository) -> None:
    """Empty username = opt-out of platform_manager bootstrap (single-
...
@pytest.mark.asyncio
async def test_platform_manager_can_login_after_bootstrap(
    tm_repo: AuthRepository,
) -> None:
