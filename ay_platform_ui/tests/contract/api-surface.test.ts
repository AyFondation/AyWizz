...
function invocations(client: ApiClient): Record<string, () => Promise<unknown> | unknown> {
  ...
  return {
    ...
    listProjectBackups: () => client.listProjectBackups(P),
    createProjectSnapshot: () => client.createProjectSnapshot(P),
    restoreProjectBackup: () => client.restoreProjectBackup(P, "b1", { dry_run: true }),
    downloadProjectBackup: () => client.downloadProjectBackup(P, "b1"),
