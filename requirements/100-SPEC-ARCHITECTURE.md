...
language: en
status: draft
derives-from: [D-002, D-003, D-007, D-008, D-010, D-011, D-012, D-013, D-014, D-020]
---
...
> projects, lifecycle, ACL, consumption within its tenant) with **no project
> content access at all** — content flows ONLY through the project-scoped
> roles. Enforced by the content components (C4-C7) stripping `admin` /
> `tenant_admin` before every content gate. `admin`-only content operations
> (run resume, KG embed, memory refresh) move to `project_owner`. RBAC
> hierarchy 6 → 5 roles; auth-matrix content endpoints drop `admin` from
> `accept_global_roles`. No migration.
>
> **Version 19 changes.** **E-100-002 v5 → v6** — adds a NEW tenant-scoped
> global role `tenant_manager` (the tenant mirror of `platform_manager`):
> full governance of its own tenant (user create/delete, project create,
> lifecycle status, ACL grant/revoke, consumption) + READ-ONLY project
> content (view, never modify). Coexists with `admin`. Read-only content
> enforced via an implicit `project_viewer` forward-auth scope + a
> method-aware guard. New role → no migration. RBAC hierarchy 5 → 6 roles.
>
> **Version 18 changes.** **E-100-002 v4 → v5** — renames the top global
> role `tenant_manager` → `platform_manager` (the role is platform-wide, not
> tenant-scoped; the old name was a misnomer). Pure rename, no power change:
> the role string in JWT `roles` / `c2_role_assignments` / every gate, the
> auth-matrix catalog, and all UI/label copy become `platform_manager`.
> Existing deployments migrate the role string in `c2_role_assignments`
> (dev re-seeds). The auth-matrix catalog + `065-TEST-MATRIX.md` are
> regenerated.
>
> **Version 17 changes.** **E-100-002 v3 → v4** — refines the content-blind
> invariant by separating a project's **governance object** (its existence,
> lifecycle status, access-control list, and resource consumption) from its
> **content** (conversations, sources, requirements, runs, artifacts). The
> `platform_manager` platform operator GAINS cross-tenant access to the governance
> object: (e) list every project across all tenants (metadata only), read a
> project's ACL + consumption, and manage its lifecycle **status**
> (`active` / `inactive` / `archived`); (f) grant/revoke a user's project access
> (assigning a `project_*` role on grant). It remains **content-blind** — it
> SHALL NOT read or write any project content. All v4 governance actions are
> **audited and reversible**. A new project `status` field (default `active`)
> is introduced; content endpoints reject non-active projects (403). The
> auth-matrix catalog + `065-TEST-MATRIX.md` are extended accordingly.

> **Version 16 changes.** **E-100-002 v2 → v3** — `platform_manager` evolves from a tenant-lifecycle-only super-root into a **platform operator**. It now owns, IN ADDITION to its v2 powers: (a) the platform **LLM registry** (D-021 — models, encrypted keys, api_base, costs, capabilities); (b) tenant lifecycle **including deactivation** (`POST /admin/tenants/{id}/deactivate` + `/reactivate` ; a deactivated tenant's members are refused login at `issue_token`); (c) **cross-tenant user OVERSIGHT** (`GET /admin/users` [optionally `?tenant_id=`], `POST /admin/users/{id}/deactivate` + `/reactivate`) — list/filter/deactivate ONLY ; user **create/delete remains the tenant's own admin** (content); (d) global LLM **quotas** (Lot 3, forthcoming). The **content-blind invariant is UNCHANGED**: `platform_manager` still has NO access to tenant CONTENT (projects, sources, conversations, requirements). The new powers are platform CONFIGURATION + account-status, never content. The auth-matrix catalog + `065-TEST-MATRIX.md` are extended accordingly.
...
> **Version 14 changes.** AyExtractor adopted as a new dependency component **C13** per `D-020`. **R-100-081 v2** re-partitions the D-013 ingestion pipeline: C12 owns trigger + orchestration, **C13** owns extract + chunk + write MinIO artifacts, C7 owns embed + index. **R-100-125** (new) declares C13: external/dependency type, vendored under `ay_extractor/`, HTTP-only surface (`POST /analyze`, `GET /status/{run_id}`, `GET /healthz`), zero code import coupling with `ay_platform_core/`, LLM-frugal stance (libraries + heuristics over LLM, image vision LLM-mandatory, decontextualiser / Refine / Chain of Density opt-in via `quality_tier`). §4.2 component table extended (4 dependency components: C10/C11/C12/C13). Detailed artifact contract + ChunkRich schema + `/ingest-chunks` C7 endpoint live in `400-SPEC-MEMORY-RAG.md` (R-400-220..225); C13 LLM agent routes live in `800-SPEC-LLM-ABSTRACTION.md` (R-800-130..133).

> **Version 13 changes.** **E-100-002 v1 → v2** : RBAC model formalised as a 5-role hierarchy. New global role **`platform_manager`** (super-root, content-blind, owns tenant lifecycle) above `admin` (= `tenant_admin`, tenant-scoped). Verification clause added — the auth × role × scope test matrix under `ay_platform_core/tests/e2e/auth_matrix/` SHALL exercise every catalogued endpoint along (anonymous, role gate, cross-tenant isolation, cross-project isolation, backend state) dimensions, with a coherence test pinning the catalog to the live route set.
...
The platform v1 comprises **10 internal components** (C1–C9, C15) and **4 external/dependency components** (C10–C13). External IdPs and LLM providers are out-of-cluster dependencies, not components.
...
| C12 | Workflow Engine | dependency | External ingestion trigger, post-release sync, automation (n8n). |
| C13 | Extraction & Chunking Service | dependency | Source extraction (PDF/EPUB/DOCX/MD/TXT/image) + chunking + optional decontextualisation/summarisation, writing markdown + jsonl + manifest artifacts to MinIO (AyExtractor, vendored under `ay_extractor/`). |
...
The v1 authorization model SHALL be role-based (RBAC) with **four** global roles (`platform_manager`, `admin` (= `tenant_admin`), `user`) and three per-project scoped roles (`project_owner`, `project_editor`, `project_viewer`). The full role-permission matrix SHALL be defined in E-100-002 v2.
...
The ingestion pipeline SHALL be implemented through the collaboration of **three** components — **C7** (Memory Service), **C12** (Workflow Engine, n8n) and **C13** (Extraction & Chunking Service, AyExtractor) — with the following exclusive responsibility split:

- **C7 SHALL own**: (a) **external-source upload reception** via `POST /api/v1/memory/projects/{project_id}/sources/upload` accepting **`multipart/form-data`** (the raw file plus its metadata: `source_id`, `mime_type`, `format`, optional `quality_tier` / `document_type`); (b) enforcement of project RBAC on the upload (`project_editor` and above, per E-100-002); (c) **writing the raw bytes to C10 (MinIO)** at `sources/{tenant_id}/{project_id}/{source_id}/raw.{format}` via C7's **authenticated MinIO client** (S3 SigV4 — no anonymous bucket access); (d) **triggering the C12 ingestion workflow** by POSTing the C12 webhook with **metadata only** (`raw_object_key`, tenant/project/source ids, filename, mime_type, format, quality_tier?) — **never the file bytes**; (e) returning `202 Accepted` so the UI can poll the source list. C7 SHALL additionally own embedding computation on the rich chunks + indexing into C11 (ArangoDB) `external_sources` index + retrieval, exposed via `POST /ingest-chunks` which **reads C13's `chunks.jsonl` + `run_manifest.json` directly from MinIO by extraction run id** (R-400-223 v3). C7 SHALL NOT parse or chunk the file synchronously (the in-process pipeline removed in v2 stays removed — the heavy extraction is C13's, run asynchronously).
- **C12 SHALL own**: receiving the **metadata-only** webhook from C7, HTTP-triggering C13 (`POST /analyze`) with the `raw_object_key`, polling C13's `status.json` to a terminal state, posting the run reference to C7 (`POST /ingest-chunks`), and publishing NATS lifecycle events (`ingestion.source.uploaded`, `ingestion.source.parsed`, `ingestion.source.indexed`, `ingestion.source.failed`). **C12 SHALL NOT receive, encode, decode, or write file bytes, and SHALL NOT read from or write to C10 (MinIO) directly.** C12 SHALL NOT perform parsing or chunking itself.
- **C13 SHALL own**: per-format parsing and text extraction (PDF / EPUB / DOCX / MD / TXT / image), structure detection, table extraction, reference extraction, optional image vision, chunking, and optional decontextualisation / Refine summarisation / Chain of Density densification (gated by the per-document `quality_tier` setting — see R-400-224). C13 reads the raw object by `raw_object_key` and SHALL write all its outputs as durable MinIO artifacts per the layout fixed in `400-SPEC-MEMORY-RAG.md` R-400-220, accompanied by a `run_manifest.json` that stamps the AyExtractor version, the per-agent LLM assignments + prompt hashes, the input fingerprint, and per-phase timing — sufficient for byte-exact reproducibility and run diff. C13 SHALL NOT read from any AyWizz store (`ay_platform_core/` code paths) and SHALL NOT be imported by any AyWizz component.

**Rationale.** **v3 (D-020 operationalisation, 2026-05-29):** the deployment surfaced that n8n (C12) is a poor custodian of file bytes — its `httpRequest` node cannot S3-SigV4-sign a MinIO write, which forced a base64-in-JSON transport, an anonymous-bucket workaround, and brittle `$json.body.*` expression plumbing (all defects, not design). Custody of the raw bytes is moved to **C7**, which already owns the sources surface, the project RBAC, and an authenticated MinIO client — restoring signed S3 writes, eliminating base64 entirely, and reducing C12 to a pure **metadata orchestrator** (it never touches a byte, end to end, since `/ingest-chunks` v3 also pulls the artifacts from MinIO inside C7). This does **not** reinstate the synchronous parse + chunk that D-020 removed from C7 — the heavy extraction stays in C13, asynchronous. **v2 (D-020):** re-partitioned the v1 split (which attributed parse + chunk to C12) so a dedicated component (C13, re-using AyExtractor) hosts parsing + chunking. C7 stays the sole owner of embeddings + indexing across all versions.

**Supersedes** R-100-081 v2 (which assigned upload reception + raw-blob MinIO write + artifact reads to C12). The pipeline stays three-step (C7-upload → C12-orchestrate → C13-extract → C7-embed/index) but byte custody moves from C12 to C7. Detailed contracts in R-400-220..225 (R-400-223 bumped to v3 for the MinIO-pull ingest).
...
**v7 makes `admin` (= `tenant_admin`) the CONTENT-BLIND tenant operator and REMOVES `tenant_manager`.** The short-lived v6 `tenant_manager` role is dropped; its purpose is absorbed by `admin`, which is redefined as the tenant-scoped mirror of `platform_manager`: full GOVERNANCE of its OWN tenant (create/delete users, create projects, project lifecycle status, ACL grant/revoke, consumption view) and **NO project CONTENT access at all** (neither read nor write). Project content is reached **ONLY** through the project-scoped roles (`project_owner` / `project_editor` / `project_viewer`). An `admin` who needs to touch a project's content must hold a project role on it. Enforcement: the content components (C4-C7) STRIP `admin` / `tenant_admin` from the caller's roles before any content gate (`_CONTENT_BLIND_GLOBAL_ROLES`), so a global admin role never satisfies a content gate; the `/admin/*` governance surface accepts `admin` scoped to its own tenant. No migration (no new role string). The v6 read-only-content model (`tenant_manager` + implicit `project_viewer` injection) is removed.

**v5 renames the top global role `tenant_manager` → `platform_manager`.** The v2–v4 name was a misnomer: the role is platform-wide (it owns the whole platform — tenants, the LLM registry, quotas, and project governance), not tenant-scoped. The role string in JWT `roles`, `c2_role_assignments`, and every gate is `platform_manager`; the login page surfaces it as the "super-root / platform manager" persona. No powers change in v5 — it is a pure rename (existing deployments migrate the role string in `c2_role_assignments`).

The platform's RBAC model comprises a **5-role hierarchy** (top to bottom). Higher roles SHALL NOT subsume lower-role permissions automatically — `platform_manager` in particular remains **content-blind** by design (separation of duties between platform operators and tenant operators). v3 expands `platform_manager` into a **platform operator** (platform configuration + account status), WITHOUT granting it any tenant CONTENT access. **v4 refines content-blindness for projects**: a project is now access-separated into a **governance object** (metadata, lifecycle status, ACL, consumption) that the operator MAY manage cross-tenant, and its **content** (what is inside) which stays blind — see *Project governance vs. content* below.

**Global roles** (stored in JWT `roles` claim):

| Role | Scope | Permissions |
|---|---|---|
| `platform_manager` | platform-wide | **Platform operator** (v3/v4). Owns: (a) tenant lifecycle — create / list / delete / **deactivate** & **reactivate** tenants (a deactivated tenant's members are refused login); grant or revoke `admin` on a tenant. (b) the platform **LLM registry** (D-021 — models, encrypted keys, api_base, cost, capabilities). (c) **cross-tenant user oversight** — `GET /admin/users` (optionally `?tenant_id=`) and **deactivate / reactivate** any user; user **create / delete remains the tenant's own `admin`** (that is tenant content). (d) global LLM **quotas**. **(e) project GOVERNANCE (v4, cross-tenant)** — **list every project across all tenants** (metadata only: id, tenant, name, status, dates, created_by); **read a project's access-control list** (its members and their `project_*` roles) and its **resource consumption** (LLM cost/tokens); **manage the project lifecycle status** — `activate` / `deactivate` / `archive` (a non-`active` project blocks all member access to its content). **(f) access GOVERNANCE (v4, cross-tenant)** — **grant or revoke a user's access to a project** (assigning a `project_*` role on grant, removing the assignment on revoke); in-place role change is out of scope (revoke + re-grant). **SHALL NOT** read or write any tenant/project CONTENT (conversations, sources, requirements, runs, artifacts), nor create/delete users — content-blindness is preserved; the operator's powers are platform configuration, account-status, and project **governance** only. |
| `admin` (= `tenant_admin`) | tenant-scoped | **Tenant operator** (v7) — the tenant-scoped mirror of `platform_manager`, CONTENT-BLIND. Within its OWN `tenant_id` ONLY: (a) **user management** — create / delete / deactivate / reactivate users of the tenant. (b) **project management** — create projects; **project GOVERNANCE** = list every project in the tenant, read each project's ACL + resource consumption, manage lifecycle status (`activate` / `deactivate` / `archive`). (c) **access GOVERNANCE** — grant / revoke a user's `project_*` role on the tenant's projects. **SHALL NOT** read or write ANY project CONTENT (conversations, sources/chunks/KG, requirements, validation runs/findings, artifacts) — content is reached ONLY through the project-scoped roles below; an `admin` who needs content must hold a project role. **SHALL NOT** cross tenants, nor touch platform configuration. Enforced by the content components stripping `admin`/`tenant_admin` before any content gate. `admin` and `tenant_admin` are synonyms — `admin` is canonical; `tenant_admin` is retained for v1 code paths. All governance actions are audited (`c2_audit`). |
...
**Permission resolution.** A user's effective permission on a resource is the union of their global roles and their project-scoped roles for that resource's project — except for `platform_manager`, whose grants apply ONLY to **platform-operator** endpoints (tenant lifecycle incl. (de)activation, the LLM registry, cross-tenant user oversight, global quotas, and — v4 — project **governance** endpoints: `/admin/projects*` list/status/ACL/consumption and cross-tenant grant/revoke), never to tenant **content** endpoints. Components SHALL enforce this by listing `platform_manager` in the role gate of platform-operator/governance endpoints only; content endpoints SHALL NOT include `platform_manager` in their accept list.

**Project governance vs. content (v4).** A project is both a *governance object* and a *content container*; the two are access-separated:

- **Governance object** — the project's *existence*, its *lifecycle status* (`active` / `inactive` / `archived`), its *access-control list* (which users hold which `project_*` role), and its *resource consumption* (LLM cost/tokens). These describe the project without revealing anything inside it. The `platform_manager` platform operator MAY read and act on the governance object **across all tenants** (per row (e)/(f) above).
- **Content** — everything *inside* the project: conversations (C3), sources / chunks / knowledge-graph (C7), requirements documents & entities (C5), validation runs & findings (C6), generated artifacts & documents (C4). `platform_manager` SHALL NOT read or write project content under any circumstance.

**Lifecycle status.** Every project carries a `status` field (default `active`). `inactive` — members are refused access to the project's content (mirrors tenant deactivation); the project and its data are retained. `archived` — a read-only freeze; content is retained but no writes, uploads, or pipeline runs are accepted. Content endpoints SHALL resolve the target project's `status` and reject a request against a non-`active` project (403), independently of the caller's role.

**Governance actions are audited and reversible.** Every v4 governance action (a project status change; a cross-tenant grant or revoke) SHALL be recorded with actor, timestamp, target, and before/after value, and SHALL be reversible (`deactivate`↔`reactivate`, `archive`↔`unarchive`, `grant`↔`revoke`). No v4 governance action performs a destructive deletion.

> **Open detail (resolved at build).** The current model is one-user-one-tenant (`UserPublic.tenant_id`). v4 project-access grant/revoke uses the existing `c2_role_assignments` mechanism (unambiguous). Operator control at the *tenant* membership level is scoped to the existing account-status power (v3 (c) deactivate/reactivate); a first-class "add existing user to another tenant" operation is NOT introduced by v4.
...
6. `platform_manager`-only endpoints reject `admin` / `project_*` roles, and vice versa.
...
### 10.9 Extraction & Chunking Service (C13, dependency)
...
status: draft
category: architecture
derives-from: [D-020, R-100-011, R-100-081]
```

The platform SHALL deploy **C13 (Extraction & Chunking Service)** as a dependency component, hosted by the AyExtractor codebase vendored under `ay_extractor/`. C13 SHALL satisfy the following invariants:

1. **Code isolation.** `ay_platform_core/` (any AyWizz component code) SHALL NOT `import ayextractor`. AyExtractor SHALL NOT `import` from `ay_platform_core`. The only contracts between C13 and AyWizz components are (a) the MinIO artifact layout defined in R-400-220 and (b) the HTTP trigger surface declared in point 2 below.

2. **HTTP surface (minimal).** C13 SHALL expose exactly three HTTP endpoints:
   - `POST /analyze` — accept an analyse request (input MinIO key + tenant_id + project_id + source_id + quality_tier + **`urgency ∈ {"interactive", "background"}`** default `"interactive"` + per-document config overrides), validate, enqueue, return `{run_id, status: "running"}` immediately (asynchronous). When `urgency="background"`, C13 SHALL route all Phase 2 LLM calls through the provider's batch API (Anthropic Batch API or equivalent via C8) — trading latency (1-24 h) for cost (-50%). The `status.json` SHALL include the resolved `urgency` value so C12 can surface a "batched" hint to the UI. `interactive` mode preserves sub-minute UX for ad-hoc uploads.
   - `GET /status/{run_id}` — return the current `RunManifest` excerpt (`status ∈ {running, completed, failed}`, `phases_completed`, `errors`).
   - `GET /healthz` — liveness/readiness probe (no auth).
   No other HTTP routes SHALL be exposed by C13 in v1. CLI invocation via the `ayextractor` entry point SHALL remain functional for dev/debug but is not the consumed surface in the cluster.

3. **Vendoring + versioning.** `ay_extractor/` SHALL stay in the monorepo; its `pyproject.toml` SHALL build a wheel locally; the C13 image (`infra/c13_extractor/`) SHALL install that wheel. The `run_manifest.json` SHALL stamp the wheel version (= `ay_extractor/src/version.py::__version__`) and the monorepo git sha at image build time, so any artifact remains attributable to an immutable AyExtractor build.

4. **LLM routing through C8.** C13 SHALL route all LLM calls through C8 (LiteLLM Gateway) per R-100-011. The implementation SHALL be a deployment-time configuration switch (`OPENAI_BASE_URL=http://c8:8000/v1` + `OPENAI_API_KEY=$C8_GATEWAY_API_KEY`) — AyExtractor's `llm/adapters/openai_adapter.py` is OpenAI-API-compatible and reaches C8 without any code change. The other provider-specific adapters (`anthropic`, `google`, `openrouter`) SHALL be physically removed from `ay_extractor/src/llm/adapters/` during the C13 refactor (D-020 session 2/3). The four AyExtractor agents (`ayextract.image_analyzer`, `ayextract.decontextualizer`, `ayextract.summarizer`, `ayextract.densifier`) SHALL appear in the C8 `agent_routes` table per R-800-130..133.

5. **LLM frugality (mandatory).** C13 SHALL default to **zero LLM calls** for documents containing no images and configured at the default `quality_tier` (`minimal`). Specifically:
   - Structure detection (TOC, sections, footnotes, bibliography) SHALL be implemented in deterministic Python (regex + indentation/font heuristics). The legacy LLM-based `structure_detector` SHALL be a removed module.
   - Reference / citation extraction SHALL use `refextract` + regex. The legacy LLM-based `reference_extractor` agent SHALL be a removed module.
   - Table extraction SHALL continue to use library-based extractors (`camelot` / `tabula-py` per format).
   - **Image deduplication (mandatory).** Before invoking `image_analyzer`, C13 SHALL compute `sha256` of each extracted image and SHALL invoke the analyser exactly **once per unique image hash within a single document**. Subsequent chunks embedding the same image SHALL reference the existing analysis result. Skips redundant Vision calls for repeated assets (footer logos, recurring icons) — 20-50% reduction on image-heavy documents at zero quality loss.
   - Image vision (`image_analyzer`) is the only **mandatory** LLM agent: every UNIQUE embedded image SHALL be analysed by an LLM-Vision model via C8. Documents with no images incur no LLM call at `quality_tier=minimal`.
   - **2-tier LLM gating for the decontextualiser (mandatory, `quality_tier=high` only).** When `quality_tier=high` selects the decontextualiser, C13 SHALL FIRST invoke a cheap screener agent `ayextract.decontextualizer_screener` (Haiku-class via C8, ~50 input + ~10 output tokens) returning `NEEDS_DECONTEXT: YES|NO`. The expensive `ayextract.decontextualizer` (Sonnet-class) SHALL be invoked **only** when the screener returns `YES`. When the screener returns `NO`, the chunk's `text` field SHALL equal its `original_text`. The screener output (verdict + brief reason) SHALL be persisted to MinIO under `02_chunks/screener_log.jsonl` for audit and eval. Catches semantic ambiguities a regex would miss while saving ~30-50% of expensive decontextualisation calls. The screener's per-call cost (Haiku) is amortised at any skip rate ≥ 2%.
   - **Mandatory `prompt_caching` structure for sliding-window agents.** The decontextualiser AND the summariser SHALL structure their prompts so that the stable prefix (running summary + sliding window of K previous chunks) is wrapped in a provider-recognised cache marker (`cache_control: {type: "ephemeral"}` for Anthropic, equivalent for OpenAI). Without this explicit marker the provider's cache does not key on the prefix and the 10× cached-input discount is forfeit. R-800-131 v2 + R-800-132 v2 make this normative.
   - `decontextualizer` (coreference resolution), `summarizer` (Refine cumulative summarisation), `densifier` (Chain of Density, 5 iterations) SHALL be **opt-in** via the `quality_tier` setting (R-400-224): `standard` enables `summarizer`; `high` enables `decontextualizer` (gated by screener per above) and `densifier`.

6. **Storage isolation.** C13 SHALL write all its outputs as MinIO objects per the layout defined in R-400-220, including the **embedding vectors** (`02_chunks/embeddings.jsonl` per R-400-222 v2). C13 SHALL NOT write to C11 (ArangoDB) — only indexing into ArangoDB remains C7's responsibility (R-100-081 v2). C7 retains its own embedder for the requirements-corpus write path (R-400-030..032); the external-source ingestion path delegates embedding compute to C13 + C8 `/embeddings`. The legacy `storage/{local_writer, s3_writer}.py` modules SHALL be replaced by a new `storage/minio_writer.py` (S3-API via `boto3`).

7. **Removed modules (no dual maintenance).** The D-020 operationalisation SHALL physically remove the following AyExtractor modules (no `extras` flag, no commented code) — they duplicate AyWizz functionality or address corpus-level concerns that belong to C7:
   - `src/rag/retriever/` (entire dir) — cross-document retrieval is C7's role.
   - `src/rag/enricher.py` — prompt enrichment from stores; AyExtractor is per-document only.
   - `src/rag/indexer.py` — push to vector store; AyExtractor writes files only.
   - `src/rag/vector_store/` (entire dir) — C11 ArangoDB is the single vector store.
   - `src/rag/graph_store/` (entire dir) — C11 ArangoDB is the single graph store.
   - `src/rag/embeddings/` (entire dir) — embeddings are C7's role.
   - `src/consolidator/` (entire dir) — cross-document consolidation is C7's role (D-016).
   - `src/batch/` (entire dir) — batch ingestion is C12's role.
   - `src/graph/` (entire dir) — KG construction is C7's role.
   - `src/pipeline/agents/{concept_extractor, community_summarizer, profile_generator, synthesizer, critic}.py` — Phase 3 agents are out of scope for D-020 v1 (queued as Q-200-022).
   - `src/cache/{sqlite,redis,arangodb}_cache_store.py` — only `json_cache_store.py` survives, backed by MinIO.

8. **Failure handling.** C13 SHALL persist intermediate phase outputs to MinIO before advancing to the next phase, so that a failure during Phase 2 does not require re-running Phase 1 (R-400-225 details the resume-from-phase semantics). On uncaught failure C13 SHALL write a final `status.json` with `{status: "failed", error_class, error_message, completed_phases}`. C12 SHALL surface this via NATS `ingestion.source.failed` to the UI.

9. **Resource limits.** C13 container SHALL declare resource requests and limits per R-100-119. Baseline budget v1: request 1 vCPU / 2 GiB, limit 4 vCPU / 8 GiB. Image vision and densification phases are the heaviest; sizing is revisited once production telemetry lands.
...
**End of 100-SPEC-ARCHITECTURE.md v15.**
