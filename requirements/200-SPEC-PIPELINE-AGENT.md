...
| Q-200-018 | Source-file metadata (R-200-173) `kg_indexed` field : v1 emits null (no C7 query path from C4). **Resolved by R-400-232 (D-021)** — the light live-docs→RAG path makes `kg_indexed` real and adds the C4→C7 ingest on create/update/delete/move. | R-400-232 | **implemented** (C7 `GET .../live-docs/kg-indexed` membership read + C4 `C7LiveDocsClient.kg_indexed` wired into the source-file meta endpoint ; R-200-173 / R-400-231/232 tested) |
